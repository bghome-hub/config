...
BEGIN
    ...
    DECLARE @TrxDate DATE, @RevDate DATE, @RevFlag CHAR(1), @Reference VARCHAR(30);
            ...
            BEGIN TRY
                ...
                BEGIN
                    EXEC taGLTransactionHeaderInsert
                        @I_vBACHNUMB = @pBatch, @I_vJRNENTRY = @JE, @I_vREFRENCE = @Ref,
...
BEGIN
    ...
    BEGIN TRY
        ...
        SET ContentHash = HASHBYTES('SHA2_256', 
            (
                SELECT Account, AccountDesc, TrxDate, DebitAmount, CreditAmount, Reference, Reversal
                FROM dbo.IC_DetailLine l
                WHERE l.HeaderId = h.HeaderId
                ORDER BY Account, AccountDesc, DebitAmount, CreditAmount, Reference, Reversal
        ...
        SELECT  
            ...
            'ERROR',
            @StepName,
           'One or more company payloads match a previously POSTED batch. Preventing POST'
            
        FROM dbo.IC_CompanyHeader current_h
        INNER JOIN IC_CompanyHeader past_h on current_h.ContentHash = past_h.ContentHash
