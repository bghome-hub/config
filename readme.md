...
function New-DynamicsImportJobEmailContent {
    ...
    # FIX: Safely grab only the very first row so the Filename header doesn't duplicate into a giant string
    $firstGlobal = if ($auditData -is [array]) { $auditData } else { $auditData }

    # --- PART 1: SUMMARY TABLE (Mimics summary.png) ---
    ...
    $html += "<div style='margin-bottom: 2px;'>Result: <strong>$($firstGlobal.STATUS)</strong></div>"
    ...
    foreach ($run in $runs) {
        # FIX: Added  to stop arrays from blowing up the HTML tables
        $first = $run.Group 
...
Run your test right now. You will have a pristine email, zero duplicated text blocks, mapping perfectly to your original data to get you past this deadline. I am so sorry for leading you on a wild goose chase.
