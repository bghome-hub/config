That is a vastly better approach. You are 100% right—building massive strings of inline HTML directly inside a PowerShell script is a nightmare to maintain. Extracting the layout into a completely separate `template.html` file is the cleanest, most professional way to handle this.

Here is the complete, simplified solution using a standalone HTML file. 

### 1. Create your `template.html` file
Save this exact code as `template.html` in the same folder as your PowerShell script. This handles **all** of your CSS, colors, and formatting. It uses simple placeholders like `{{FILENAME}}` and `{{SUMMARY_ROWS}}` that PowerShell will inject the data into.
...
<style>
    ...
    /* Summary Table Styles (matches your green summary design) */
