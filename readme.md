...
BEGIN
    ...
    BEGIN TRY
        ...
        IF EXISTS (
            SELECT 1 FROM dbo.IC_StagingRaw 
            WHERE JobId = @JobId AND TRY_CAST(Amount AS DECIMAL(19,2)) IS NULL -- FIX APPLIED HERE
        )
        BEGIN
            UPDATE dbo.IC_ImportJob SET JobStatus = 'FAILED' WHERE JobId = @JobId;
            SET @LogMsg = 'Amount value is in incorrect format (Non-numeric). Entire job rejected.';
            INSERT INTO dbo.IC_SystemLog (JobId, LogLevel, StepName, LogMessage) VALUES (@JobId, 'FATAL', @StepName, @LogMsg);
            RETURN;
        END

        SET @LogMsg = 'Phase 1 Complete: File passed all FATAL string checks.';
        INSERT INTO dbo.IC_SystemLog (JobId, LogLevel, StepName, LogMessage) VALUES (@JobId, 'INFO', @StepName, @LogMsg);
        RAISERROR(@LogMsg, 0, 1) WITH NOWAIT;
        ...
        SET @RowCnt = @@ROWCOUNT;
        SET @LogMsg = 'Phase 2A Complete: Promoted ' + CAST(@RowCnt AS VARCHAR(10)) + ' distinct company headers.';
        INSERT INTO dbo.IC_SystemLog (JobId, LogLevel, StepName, LogMessage) VALUES (@JobId, 'INFO', @StepName, @LogMsg);
        RAISERROR(@LogMsg, 0, 1) WITH NOWAIT;
        ...
        SET @RowCnt = @@ROWCOUNT;
        SET @LogMsg = 'Phase 2B Complete: Promoted ' + CAST(@RowCnt AS VARCHAR(10)) + ' detail lines.';
        INSERT INTO dbo.IC_SystemLog (JobId, LogLevel, StepName, LogMessage) VALUES (@JobId, 'INFO', @StepName, @LogMsg);
        RAISERROR(@LogMsg, 0, 1) WITH NOWAIT;
        ...
        IF @RowCnt > 0
        BEGIN
            SET @LogMsg = 'Phase 3 Warn: ' + CAST(@RowCnt AS VARCHAR(10)) + ' company payloads match previously POSTED batches. Marked SKIPPED.';
        ...
        BEGIN
            ...
            SET @LogMsg = 'Phase 4 Warn: ' + CAST(@RowCnt AS VARCHAR(10)) + ' batches failed TrxDate check.';
            RAISERROR(@LogMsg, 0, 1) WITH NOWAIT;
        END
        ...
        BEGIN
            ...
            SET @LogMsg = 'Phase 4 Warn: ' + CAST(@RowCnt AS VARCHAR(10)) + ' batches failed Balance check.';
            RAISERROR(@LogMsg, 0, 1) WITH NOWAIT;
        END
        ...
        BEGIN
            SET @LogMsg = 'Executing cross-db checks against company DB: ' + @DynamicCo;
            RAISERROR(@LogMsg, 0, 1) WITH NOWAIT;
